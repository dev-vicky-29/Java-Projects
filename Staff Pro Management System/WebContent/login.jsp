...
if(request.getParameter("btnlogin")!=null) {
    ...
    if(user.equals("Admin")) {
        ...
        if(rs.next()) {
            ...
            response.sendRedirect("/Staff_Pro_Management_System/Admin/index.jsp");
        } else {
        ...
        ResultSet rs = db.getRows("select * from staff where EmailID=? and Password=?", mail, pass);
        if(rs.next()) {
            session.setAttribute("user", user);
            session.setAttribute("mail", mail);
            session.setAttribute("pass", pass);
            session.setAttribute("type", "staff");
            response.sendRedirect("/Staff_Pro_Management_System/Staff/index.jsp");
        } else {
            session.setAttribute("user", null);
            session.setAttribute("mail", null);
            session.setAttribute("pass", null);
