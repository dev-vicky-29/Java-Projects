...
{	
	...
	ResultSet rs=db.getRows("select * from Client where Password=? and EmailID=?", opass,session.getAttribute("user").toString());
	if(rs.next())
	{
		db.execute("Update Client set Password=? where EmailID=?", npass,session.getAttribute("user").toString());
