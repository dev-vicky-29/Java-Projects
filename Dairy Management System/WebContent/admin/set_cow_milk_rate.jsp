...
<%
    if (request.getParameter("btnadd") != null) {
        String milkType = "Cow"; // Set milk type to Cow
        ...
        if (rate <= 0) {
            ...
            ResultSet rs = db.getRows("SELECT * FROM cow_milk_rates WHERE FatPercentage=?", fatPercentage);
            if (rs.next()) {
                // If FatPercentage exists, display confirmation popup
                %>
                <script>
                    if (confirm("Fat Percentage already exists for Cow Milk. Do you want to update it?")) {
                        window.location.href = "update_cow_milk_rate.jsp?fatPercentage=<%=fatPercentage%>&rate=<%=rate%>";
                    }
                </script>
                <%
            } else {
                ...
                int rowsAffected = db.executeSql("INSERT INTO cow_milk_rates (FatPercentage, Rate) VALUES (?, ?)", fatPercentage, rate);

                if (rowsAffected > 0) {
                    successMsg = "Cow Milk Rate Added Successfully";
                } else {
                    errorMsg = "Failed to add Cow Milk Rate";
                }
            }
        }
    }
...
<div class="card mt-5 col-7 offset-2">
    <div class="card-header bg-info text-white">
        <h1 class="text-center">Set Cow Milk Rate</h1>
    </div>
