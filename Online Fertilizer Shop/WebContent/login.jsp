...
if(request.getParameter("btnlogin")!=null){
	...
	if(user.equals("Admin")){
		...
		if(rs.next()){
			...
			response.sendRedirect("/Online_Fertilizer_Shop/Admin/admin.jsp");
		} else{
			...
			redirectUrlError="/Online_Fertilizer_Shop/login.jsp";
		}
	}else if(user.equals("Customer")){
		ResultSet rs=db.getRows("select * from customerlogin where EmailID=? and Password=?", mail,pass);
		if(rs.next()){
			session.setAttribute("user", user);
			session.setAttribute("mail", mail);
			session.setAttribute("pass", pass);
			session.setAttribute("type", "customer");
			response.sendRedirect("/Online_Fertilizer_Shop/Customer/customer.jsp");
		} else {
			session.setAttribute("user", null);
			session.setAttribute("mail", null);
			session.setAttribute("pass", null);
    ...
    <form method="post" id="form1" class="form custom-border mt-8 rounded">
        <div class="col-6 offset-3">
            ...
            <div class="text-center border p-3 hover-effect mt-3">
                ...
                <a href="/Online_Fertilizer_Shop/signup.jsp" class="d-inline" style="font-weight: bold;">Sign Up</a>
            </div>
        </div>
    </form>
</div>
