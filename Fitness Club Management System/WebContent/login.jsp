...
if(request.getParameter("btnlogin")!=null) {
    ...
    if(user.equals("Admin")) {
        ...
        if(rs.next()) {
            ...
            response.sendRedirect("/Fitness_Club_Management_System/Admin/admin.jsp");
        } else {
        ...
        ResultSet rs = db.getRows("select * from trainer where Email=? and Password=?", mail, pass);
        if(rs.next()) {
            session.setAttribute("user", user);
            session.setAttribute("mail", mail);
            session.setAttribute("pass", pass);
            session.setAttribute("type", "trainer");
            response.sendRedirect("/Fitness_Club_Management_System/Trainer/trainer.jsp");
        } else {
            session.setAttribute("user", null);
            session.setAttribute("mail", null);
            session.setAttribute("pass", null);
        ...
        ResultSet rs = db.getRows("select * from Client where EmailId=? and Password=?", mail, pass);
        if(rs.next()) {
            session.setAttribute("user", user);
            session.setAttribute("mail", mail);
            session.setAttribute("pass", pass);
            session.setAttribute("type", "client");
            response.sendRedirect("/Fitness_Club_Management_System/Client/client.jsp");
        } else {
            session.setAttribute("user", null);
            session.setAttribute("mail", null);
            session.setAttribute("pass", null);
