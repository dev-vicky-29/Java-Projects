...
if(request.getParameter("btnlogin")!=null){
	...
	if(user.equals("Admin")){
		ResultSet rs=db.getRows("select * from adminlogin where UserName=? and Password=? ", mail,pass);
		if(rs.next()){
			session.setAttribute("user", user);
			session.setAttribute("mail", mail);
			session.setAttribute("pass", pass);
			session.setAttribute("type", "admin");
			response.sendRedirect("/Online_Cloth_Shop/Admin/admin.jsp");
		} else{
			...
			redirectUrlError="/Online_Cloth_Shop/login.jsp";
		}
		...
		if(rs.next()){
			...
			response.sendRedirect("/Online_Cloth_Shop/Customer/customer.jsp");
		} else {
			session.setAttribute("user", null);
			session.setAttribute("mail", null);
			session.setAttribute("pass", null);
    ...
    <form method="post" id="form1" class="form custom-border mt-8 rounded">
        <div class="col-6 offset-3">
            ...
            <div class="text-center mt-3">
                <a class="text-center text-decoration-none" style="color: black;" href="/Online_Cloth_Shop/forgotpassword.jsp">Forgot Password?</a>
            </div> 
            <hr/>
            <div class="text-center border p-3 hover-effect mt-3">
                ...
                <a href="/Online_Cloth_Shop/signup.jsp" class="d-inline" style="font-weight: bold;">Sign Up</a>
            </div>
        </div>
    </form>
</div>
