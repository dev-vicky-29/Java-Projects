...
if(request.getParameter("btnsignup") != null){
    ...
    if (!email.matches(emailRegex)) {
        ...
        String subject = "Welcome to Online Book Shop!";
        String emailBody = "Dear " + name + ",\n\n"
                + "Thank you for signing up with Online Book Shop. You can now browse and purchase books easily.\n\n"
                + "Your registered email: " + email + "\n"
                + "Your registered phone: " + phone + "\n\n"
                + "Best Regards,\nOnline Book Shop Team";

        try {
            SendMail.send(email, subject, emailBody);
            successMsg = name + " You have successfully created an account! Please login now.";
            redirectUrlSuccess = "/Online_Book_Shop/login.jsp";
        } catch (Exception e) {
            errorMsg = "Account created, but email confirmation could not be sent.";
        }
    }
}
%>
    ...
    <form method="post" id="form1" class="form custom-border mt-5 rounded" onsubmit="return validateForm()">
        <div class="col-6 offset-3">
            ...
            <div class="text-center mt-3">
                <span>Already have an account? </span><a href="/Online_Book_Shop/login.jsp">Login</a>
            </div> 
        </div>
    </form>
</div>
